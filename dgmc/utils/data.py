...
class ValidPairDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return PairData(
            ...
            edge_index_s=data_s.edge_index,
            edge_attr_s=data_s.edge_attr,
            has_spectral_s=data_s.has_spectral if hasattr(
                data_s, 'has_spectral') else None,
            has_laplacian_s=data_s.has_laplacian if hasattr(
                data_s, 'has_laplacian') else None,
            mass_s=data_s.mass if hasattr(data_s, 'mass') else None,
            evals_s=data_s.evals if hasattr(data_s, 'evals') else None,
            evecs_s=data_s.evecs if hasattr(data_s, 'evecs') else None,
            laplacian_s=data_s.laplacian
            if hasattr(data_s, 'laplacian') else None,
            x_t=data_t.x,
            idx_t=idx_t,
            edge_index_t=data_t.edge_index,
            edge_attr_t=data_t.edge_attr,
            has_spectral_t=data_t.has_spectral if hasattr(
                data_t, 'has_spectral') else None,
            has_laplacian_t=data_t.has_laplacian if hasattr(
                data_t, 'has_laplacian') else None,
            mass_t=data_t.mass if hasattr(data_t, 'mass') else None,
            evals_t=data_t.evals if hasattr(data_t, 'evals') else None,
            evecs_t=data_t.evecs if hasattr(data_t, 'evecs') else None,
            laplacian_t=data_t.laplacian
            if hasattr(data_t, 'laplacian') else None,
            y=y,
            num_nodes=None,
        )
