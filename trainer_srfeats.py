...
from utils.loss_fmap import SoftFmapLoss
...
class Trainer(object):
    ...
    def _init_train(self, phase='train'):
        ...
        cfg.log_dir = osp.join(cfg.log_dir, f'SRFeat-S_dfn_{log_dir_prefix}_{exp_time}')
        ...
        fmap_type = cfg.loss.fmap_type
        if fmap_type == 'soft_fmap':
            self.loss_fmap = SoftFmapLoss(cfg.loss.spectral_dims, cfg.loss.fmap_maxval)
        else:
            raise NotImplementedError
        ...
        wandb.init(project=cfg.project,
                   ...
                   group=cfg.group,
                   notes=exp_time,
                   tags=[exp_time[:8], contrast_type, fmap_type, cfg.model.name] + data_names,
    ...
    def _train_epoch(self, epoch, phase='train'):
        ...
        for iter_idx in pbar:
            ...
            loss_dict_f = self.loss_fmap(
                batch_data['evecs0'].float(),
                batch_data['evecs1'].float(),
                batch_data['evals0'].float(),
                batch_data['evals1'].float(),
                batch_data['mass0'].float(),
                batch_data['mass1'].float(),
                feats0,
                feats1,
                batch_data['corr'].long(),
            )
            loss_dict = {**loss_dict_c, **loss_dict_f}

            loss = cfg.loss.contrast_weight * loss_dict['contrast'] + \
                   get_weight(cfg.loss.fmap_weight, cfg.loss.fmap_silent_steps) * loss_dict['fmap']
