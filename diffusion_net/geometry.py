...
def mesh_vertex_normals(verts, faces):
    # numpy in / out
    face_n = toNP(face_normals(torch.tensor(verts),
                               torch.tensor(faces)))  # ugly torch <---> numpy
...
def vertex_normals(verts, faces, n_neighbors_cloud=30):
    ...
    if faces.numel() == 0:  # point cloud
        ...
        if bad_normals_mask.any():
            normals[bad_normals_mask, :] = (np.random.RandomState(seed=777).rand(*verts.shape) -
...
def build_tangent_frames(verts, faces, normals=None):
    ...
    basisX = torch.where((torch.abs(dot(vert_normals, basis_cand1)) < 0.9).unsqueeze(-1),
...
def get_operators(verts,
    ...
    if op_cache_dir is not None:
        ...
        while True:
            ...
            search_path = os.path.join(op_cache_dir,
            ...
            try:
                ...
                if (not np.array_equal(verts, cache_verts)) or (not np.array_equal(
                        faces, cache_faces)):
                    i_cache_search += 1
...
def geodesic_label_errors(opts,
                          target_verts,
                          target_faces,
                          pred_labels,
                          gt_labels,
    ...
    if normalization == 'diameter':
        ...
        total_area = torch.sum(face_area(torch.tensor(target_verts),
...
def get_all_pairs_geodesic_distance(opts, verts_np, faces_np):
    ...
    if opts.geodesic_cache_dir is not None:
        ...
        while True:
            ...
            # Form the name of the file to check
            search_path = os.path.join(opts.geodesic_cache_dir,
                                       hash_key_str + "_" + str(i_cache_search) + ".npz")
            ...
            try:
                ...
                # If the cache doesn't match, keep looking
                if (not np.array_equal(verts_np, cache_verts)) or (not np.array_equal(
                        faces_np, cache_faces)):
                    i_cache_search += 1
                    continue
    ...
    if not found:
        ...
        result_dists = np.nan_to_num(result_dists,
                                     nan=max_dist,
                                     posinf=max_dist,
                                     neginf=max_dist)
